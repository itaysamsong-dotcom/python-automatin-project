...
from src.utils.checkout_utils import (
    add_items_to_cart,
    assert_cart_total_not_exceeds,
    check_product_creation,
    proceed_to_checkout,
)
...
def test_buying_product_saves_invoice(product_page: Page, product, product_id: str) -> None:
    add_items_to_cart(product_page, product_id, product)
    assert_cart_total_not_exceeds(product_page, product.unit_price, 2)
